...
class CMB2_Field_Animation {
    ...
    public function render( $field, $value, $object_id, $object_type, $field_type ) {
        ...
        if( $field->args( 'preview' ) ) {
            echo '<input class="cmb-animation-preview button-secondary" value="' . __( 'Animate it', 'cmb2' ) . '" type="button">';
        }
     ...
     * Edit: Remove wp_register_script
     * Edit: Include minified version
     * Edit: Remove custom css file and added inline
     */
    public function setup_admin_scripts() {
        wp_enqueue_style( 'animate-css', plugins_url( 'css/animate.min.css', __FILE__ ), array(), self::VERSION );
        wp_enqueue_script( 'cmb-animation', plugins_url( 'js/cmb-animation.js', __FILE__ ), array( 'jquery' ), self::VERSION, true );
    }
